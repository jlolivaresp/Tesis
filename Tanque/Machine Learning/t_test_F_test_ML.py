...
# Procedemos a realizar la deteccion de fallas
...
ax[0].text(([i for i in range(0, lags+1)]*n_neighbors)[max_FDR[0]], max_FDR[1]+0.001,
           '{:.4f}'.format(max_FDR[1]), ha='center', va='bottom', fontdict=font, color='#108282')
ax[0].text(([i for i in range(0, lags+1)]*n_neighbors)[max_FDR[0]], max_FDR[1]-0.001,
           '{:.4f}'.format(FAR_for_max_FDR), ha='center', va='top', fontdict=font, color='#ad190f')
ax[1].text(([i for i in range(0, lags+1)]*n_neighbors)[min_FAR[0]], min_FAR[1],
           '{:.4f}'.format(min_FAR[1]), ha='right', va='top', fontdict=font, color='#ad190f')
ax[1].text(([i for i in range(0, lags+1)]*n_neighbors)[min_FAR[0]], min_FAR[1],
           '{:.4f}'.format(FDR_for_min_FAR), ha='right', va='bottom', fontdict=font, color='#108282')
